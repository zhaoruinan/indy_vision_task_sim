...
from pybullet_tools.indy_primitives import BodyPose, BodyConf, Command, get_grasp_gen, \
...
def main(display='execute'): # control | execute | step
    ...
    draw_global_system()
    with HideOutput():
        robot = load_model('../indy7_pybullet/indy7_base.urdf') # INDY_URDF 
    ...
    command = plan(robot, block, fixed=[floor], teleport=True)
    ...
    if display == 'control':
        ...
        command.refine(num_steps=300).execute(time_step=0.005)
    elif display == 'step':
        command.step()
    else:
